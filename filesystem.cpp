...
namespace filesystem {
	...
	FileDescriptor FileSystem::_getDescriptorByIndex(int fd_index) {
		...
		RawDiskReader fin(&ios, read_block_idx, shift);
	...
	*/
	...
	int FileSystem::_writeToFile(OFTEntry* entry, FileDescriptor& fd, void* read_ptr, int bytes)
	{
		...
		if (_reserveBytesForFile(&fd, bytes_to_alloc) == RetStatus::FAIL) {
			...
			if (entry->fpos > fd.file_length) {
				...
				RawDiskWriter fout(&ios, fd_block_idx, fd_offset);
	...
	std::optional<DirectoryEntry> FileSystem::_findFileInDirectory(const char* filename)
	{
	...
	int FileSystem::createFile(const char* filename)
	{
		...
		auto dir_entry = _findFileInDirectory(filename);
		if (num_files_created == FD_CREATED_LIMIT || dir_entry)
			return RetStatus::FAIL;
		...
		RawDiskReader fin(&ios, block_idx, offset);
		...
		_lseek(dir, dir_fd, num_files_created * sizeof(DirectoryEntry));
		...
		if (_writeToFile(dir, dir_fd, &new_entry, sizeof(DirectoryEntry))) {
			...
			RawDiskWriter fout(&ios, free_fd_block_idx, free_fd_offset);
	...
	int FileSystem::destroyFile(const char* filename)
	{
		...
		int oft_index = oft.getOftIndex(dir_entry->fd_index);
		if (!dir_entry || oft_index == -1) {
			return RetStatus::FAIL;
		}
		...
		FileDescriptor fd = _getDescriptorByIndex(dir_entry->fd_index);
		int num_of_occupied_blocks = (fd.file_length + BLOCK_SIZE - 1) / BLOCK_SIZE;
		...
		//free occupied file descriptor on the disk
		int bytes = sizeof(std::bitset<DISC_BLOCKS_NUM>) + sizeof(components::FileDescriptor)*dir_entry->fd_index;
		int block_idx = bytes / BLOCK_SIZE, offset = bytes % BLOCK_SIZE;
		RawDiskWriter fout(&ios, block_idx, offset);
		...
		int remove_file_offset = sizeof(DirectoryEntry) * dir_entry->fd_index;
		...
		if (bitset_is_modified) {
			...
			RawDiskWriter f(&ios, block_idx, offset);
			f.write(&dir_fd, sizeof(FileDescriptor));
		}
		return RetStatus::OK;
	}
	...
	int FileSystem::open(const char* filename)
	{
		...
		int oft_index = oft.addFile(dir_entry->fd_index);
		if (oft_index == -1) {
			return -1;
		}
		FileDescriptor fd = _getDescriptorByIndex(dir_entry->fd_index);
